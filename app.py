    ...
    with st.spinner("🔍 Processing your response..."):
        try:
            ...
            if len(st.session_state.messages) <= 1:
                ...
                prompt_text = f"You are a friendly scholarship assistant. Welcome the user and ask: '{next_question}'. Keep it warm and encouraging."
            elif st.session_state.current_question < len(questions):
                st.session_state.current_question += 1
                next_question = questions[st.session_state.current_question - 1]
                prompt_text = f"You are a scholarship assistant. Briefly acknowledge their answer: '{prompt}' and then ask the next question: '{next_question}'. Be encouraging and friendly."
            else:
                # Analysis phase
                user_answers = []
                ...
                answers_summary = "\n".join(user_answers)
                prompt_text = f"""Based on this student's profile, find ALL scholarships they are eligible for from the knowledge base. Present the results in a clear, organized format.

Student Profile:
{answers_summary}

Please:
1. List each qualifying scholarship with its name
2. Explain why they qualify (specific requirements they meet)
3. Include application deadlines and award amounts if available
4. Provide application instructions or next steps
5. Be encouraging and specific about their qualifications

Do not mention scholarships they don't qualify for."""
            ...
            if st.session_state.current_question > len(questions):
                st.balloons()
                st.success("🎉 Analysis complete! Check out your personalized scholarship matches above.")
